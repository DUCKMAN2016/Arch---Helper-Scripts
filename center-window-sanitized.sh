...
# Center window using KDE window manager
# This script waits for a window to launch and then centers it
# Customize WINDOW_NAME to match your target window

# TODO: Set this to your window name (partial match works)
WINDOW_NAME="MAME"  # Change this to match your window (e.g., "MAME", "DOSBox", "RetroArch", etc.)

MAX_WAIT=10
WAIT_COUNT=0

echo "Waiting for window to appear..."
echo "Target window name: $WINDOW_NAME"
echo ""
...
while [ $WAIT_COUNT -lt $MAX_WAIT ]; do
    ...
    if [ -n "$WINDOW_ID" ]; then
        ...
        # Default window size (adjust as needed)
        WINDOW_WIDTH=1024
        WINDOW_HEIGHT=768
...
echo "Timeout: Could not find window after ${MAX_WAIT} seconds"
echo ""
echo "Troubleshooting:"
echo "1. Make sure the WINDOW_NAME variable matches your target window"
echo "2. Use 'xwininfo -tree' to see all window names"
echo "3. You can use partial matches (e.g., 'MAME' matches 'DEC Rainbow MAME')"
echo "4. Increase MAX_WAIT if the window takes longer to appear"
exit 1
